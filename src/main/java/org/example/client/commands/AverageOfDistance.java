...
public class AverageOfDistance implements Command {
    public void executeCommand(String[] args, SocketChannel serverChannel) {
        ...
        try {
            writeModule.writePacketForServer(serverChannel,
                    new CommandPacket("average_of_distance", args, null, Client.getLogin(), Client.getPassword_hash()));
        } catch (IOException e) {
    ...
    public String toString() {
        return "average_of_distance - возвращает среднее арифметическое значение поля distance";
    }
}